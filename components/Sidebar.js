...
export default function Sidebar({ open, setOpen }) {
  ...
  // Optional: auto-open sidebar on desktop resize
  useEffect(() => {
    function handleResize() {
      if (!isMobile() && setOpen) setOpen(true);
    }
  ...
  const handleRandomClick = useCallback(async (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (isMobile() && setOpen) setOpen(false); // Close sidebar on mobile
  ...
  // Add a class to hide sidebar on mobile if closed
  const sidebarClass = ['sidebar'];
  if (typeof open === 'boolean' && !open && isMobile()) sidebarClass.push('sidebar-closed');

  // Overlay for mobile when sidebar is open
  const showOverlay = typeof open === 'boolean' && open && isMobile();
    ...
    <>
      {showOverlay && (
        <div
          ...
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.5)', zIndex: 1100 }}
          onClick={() => setOpen(false)}
        />
      )}
      <nav className={sidebarClass.join(' ')} style={showOverlay ? { zIndex: 1200, position: 'fixed', top: 0, left: 0, height: '100vh', background: 'var(--secondary-bg)' } : {}}>
